...
for (i in 1:length(seeds)) {
  ...
  for (ct in celltypes) {
    ...
    design <- model.matrix(~ pbc.metaD$fk.tr[pbc.metaD$cg_cov==ct]+ 
                             ...
                             pbc.metaD$fk.tr[pbc.metaD$cg_cov==ct]*pbc.metaD$pop_cov[pbc.metaD$cg_cov==ct] ) 
    
    colnames(design) <- c('Intercept','tr',paste0('Proc', 2:4),'pop', 'interaction')
    v <- voom(y.ct, design, plot=F) 
    vfit <- lmFit(v, design)
    
    alphaUQB <- vfit$coefficients[,3:5] # same coefficients as efit and returned by toptable
    newY <-t(v$E) - design[,3:5]%*%t(alphaUQB)
    ...
    UQBDE[[ct]] <- limma.matrixint(design=design,vfit=vfit, gene.D=gene.D.ct)
    ...
    ruv3DE[[ct]] <- limma.matrixint(design=design,vfit=vfit, gene.D=gene.D.ct)
    ...
    design <- model.matrix(~ pbc.metaD$fk.tr[pbc.metaD$cg_cov==ct]+ ruv3$W + 
                             ...
                             pbc.metaD$pop_cov[pbc.metaD$cg_cov==ct]+
                             pbc.metaD$fk.tr[pbc.metaD$cg_cov==ct]*pbc.metaD$pop_cov[pbc.metaD$cg_cov==ct]) 
    colnames(design) <- c('Intercept','tr',paste0('W', 1:k), paste0('PCohort', 2:4),'pop','interaction')
    ...
    ruv3BDE[[ct]] <-  limma.matrixint(design=design,vfit=vfit, gene.D=gene.D.ct)
    ...
    design <- model.matrix(~ sifull$fk.tr[sifull$sample_cell%in%orig.s]+
                             ruv3ps$W[orig.s,]+
                             sifull$pop_cov[sifull$sample_cell%in%orig.s]+
                             sifull$fk.tr[sifull$sample_cell%in%orig.s]*sifull$pop_cov[sifull$sample_cell%in%orig.s]) 
    colnames(design) <-c('Intercept','tr',paste0('W', 1:k),'pop','interaction')
    
    v <- voom(ps.y.ct[,sifull$sample_cell[sifull$cg_cov==ct]%in%orig.s], design, plot=F) 
    vfit <- lmFit(v, design)
    ...
    ruv3psDE[[ct]] <- limma.matrixint(design=design, vfit=vfit, gene.D=gene.D.ct)
    ...
    ruv3psallDE[[ct]] <- limma.matrixint( design=design, vfit=vfit, gene.D=gene.D.ct)
...
saveRDS(truthits,paste0(path,'/truthpbps10_dgim_popEint.rds'))
saveRDS(pvalits,paste0(path,'/pvalspbps10_dgim_popEint.rds'))
saveRDS(aswits,paste0(path,'/aswpbps10_dgim_popEint.rds'))
