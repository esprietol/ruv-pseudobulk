...
for (i in 1:length(seeds)) {
  ...
  for (ct in celltypes) {
    ...
    pco <- as.integer(as.factor(PBC$samples$fk.tr[PBC$samples$cg_cov==ct]))
    sil2 <- silhouette(x=pco,dist= dist(pca[,1:2], "euclidean"))
    silp <- silhouette(x=pco,dist= dist(pca, "manhattan"))
    ...
    design <- model.matrix(~ PBC$samples$fk.tr[PBC$samples$cg_cov==ct]+ PBC$samples$pop_cov[PBC$samples$cg_cov==ct]+ PBC$samples$Processing_Cohort[PBC$samples$cg_cov==ct] +PBC$samples$fk.tr[PBC$samples$cg_cov==ct]*PBC$samples$pop_cov[PBC$samples$cg_cov==ct]) 
    colnames(design) <- c('Intercept','tr','pop',paste0('Proc', 2:4), 'interaction')
    ...
    efit <- eBayes(vfit)
    etable <- topTable(efit, number=dim(y.ct)[1], adjust="BH")
    etable <- etable[gene.D.ct$gene,]
    etable$DEG <- ifelse(etable$adj.P.Val<0.01,T,F)
    etable$trueDEG <- gene.D.ct$trueDE
    ...
    design <- model.matrix(~ PBC$samples$fk.tr[PBC$samples$cg_cov==ct]+PBC$samples$pop_cov[PBC$samples$cg_cov==ct]+ ruv2T1$W[PBC$samples$cg_cov==ct,]+PBC$samples$fk.tr[PBC$samples$cg_cov==ct]*PBC$samples$pop_cov[PBC$samples$cg_cov==ct] ) 
    colnames(design) <- c('Intercept','tr','pop',paste0('W', 1:k),'interaction')
    v <- voom(y.ct, design, plot=F) 
    vfit <- lmFit(v, design)
    contrast.matrix <- makeContrasts(
      SLEeuro =tr,
    ...
    design <- model.matrix(~ PBC$samples$fk.tr[PBC$samples$cg_cov==ct]+ ruv2$W +PBC$samples$pop_cov[PBC$samples$cg_cov==ct]+PBC$samples$fk.tr[PBC$samples$cg_cov==ct]*PBC$samples$pop_cov[PBC$samples$cg_cov==ct]) 
    ...
    design <- model.matrix(~ PBC$samples$fk.tr[PBC$samples$cg_cov==ct]+PBC$samples$pop_cov[PBC$samples$cg_cov==ct]+ ruv2T3$W[indsamp.ct,]+PBC$samples$fk.tr[PBC$samples$cg_cov==ct]*PBC$samples$pop_cov[PBC$samples$cg_cov==ct] ) 
    colnames(design) <- c('Intercept','tr','pop',paste0('W', 1:k),'interaction')
    v <- voom(y.ct, design, plot=F) 
    vfit <- lmFit(v, design)
    contrast.matrix <- makeContrasts(
    ...
    design <- model.matrix(~ PBC$samples$fk.tr[PBC$samples$cg_cov==ct]+PBC$samples$pop_cov[PBC$samples$cg_cov==ct]+ ruv3T1$W[PBC$samples$cg_cov==ct,]+PBC$samples$fk.tr[PBC$samples$cg_cov==ct]*PBC$samples$pop_cov[PBC$samples$cg_cov==ct] ) 
    colnames(design) <- c('Intercept','tr','pop',paste0('W', 1:k),'interaction')
    v <- voom(y.ct, design, plot=F) 
    vfit <- lmFit(v, design)
    contrast.matrix <- makeContrasts(
    ...
    design <- model.matrix(~ PBC$samples$fk.tr[PBC$samples$cg_cov==ct]+PBC$samples$pop_cov[PBC$samples$cg_cov==ct]+ ruv3$W + PBC$samples$fk.tr[PBC$samples$cg_cov==ct]*PBC$samples$pop_cov[PBC$samples$cg_cov==ct] ) 
    colnames(design) <- c('Intercept','tr','pop',paste0('W', 1:k),'interaction')
    v <- voom(y.ct, design, plot=F) 
    vfit <- lmFit(v, design)
    contrast.matrix <- makeContrasts(
    ...
    design <- model.matrix(~ PBC$samples$fk.tr[PBC$samples$cg_cov==ct]+PBC$samples$pop_cov[PBC$samples$cg_cov==ct]+ ruv3T3$W[indsamp.ct,]+PBC$samples$fk.tr[PBC$samples$cg_cov==ct]*PBC$samples$pop_cov[PBC$samples$cg_cov==ct] ) 
    colnames(design) <- c('Intercept','tr','pop',paste0('W', 1:k),'interaction')
    v <- voom(y.ct, design, plot=F) 
    vfit <- lmFit(v, design)
    contrast.matrix <- makeContrasts(
    ...
    design <- model.matrix(~ PBC$samples$fk.tr[PBC$samples$cg_cov==ct]+PBC$samples$pop_cov[PBC$samples$cg_cov==ct]+ ruv4T1$W[PBC$samples$cg_cov==ct,]+PBC$samples$fk.tr[PBC$samples$cg_cov==ct]*PBC$samples$pop_cov[PBC$samples$cg_cov==ct] ) 
    colnames(design) <- c('Intercept','tr','pop',paste0('W', 1:k),'interaction')
    v <- voom(y.ct, design, plot=F) 
    vfit <- lmFit(v, design)
    contrast.matrix <- makeContrasts(
    ...
    design <- model.matrix(~ PBC$samples$fk.tr[PBC$samples$cg_cov==ct]+PBC$samples$pop_cov[PBC$samples$cg_cov==ct]+ ruv4$W+PBC$samples$fk.tr[PBC$samples$cg_cov==ct]*PBC$samples$pop_cov[PBC$samples$cg_cov==ct] ) 
    colnames(design) <- c('Intercept','tr','pop',paste0('W', 1:k),'interaction')
    v <- voom(y.ct, design, plot=F) 
    vfit <- lmFit(v, design)
    contrast.matrix <- makeContrasts(
    ...
    design <- model.matrix(~ PBC$samples$fk.tr[PBC$samples$cg_cov==ct]+PBC$samples$pop_cov[PBC$samples$cg_cov==ct]+ ruv4T3$W[indsamp.ct,]+PBC$samples$fk.tr[PBC$samples$cg_cov==ct]*PBC$samples$pop_cov[PBC$samples$cg_cov==ct] ) 
    colnames(design) <- c('Intercept','tr','pop',paste0('W', 1:k),'interaction')
    v <- voom(y.ct, design, plot=F) 
    vfit <- lmFit(v, design)
    contrast.matrix <- makeContrasts(
...
saveRDS(pvals,paste0(path,'/pvals_ruvdg_diffg_pop_Eint.rds'))

saveRDS(sw2its,paste0(path,'/asw2_ruvdg_diffg_pop_Eint.rds'))
saveRDS(swpits,paste0(path,'/aswp_ruvdg_diffg_pop_Eint.rds'))

saveRDS(sw2itstr,paste0(path,'/asw2tr_ruvdg_diffg_pop_Eint.rds'))
saveRDS(swpitstr,paste0(path,'/aswptr_ruvdg_diffg_pop_Eint.rds'))
